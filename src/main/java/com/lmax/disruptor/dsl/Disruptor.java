...
public class Disruptor<T>
{
    ...
    /**
     * @param barrierSequences  表示当前消费者组的序列数组，如果当前消费者组是第一组，则取一个空的序列数组；否则就是上一组消费者组的序列数组
     * @param eventHandlers     是代表事件消费逻辑的EventHandler数组。实际的消费者
     * */
        ...
        final EventHandler<? super T>[] eventHandlers)
    {
        ...
        for (int i = 0, eventHandlersLength = eventHandlers.length; i < eventHandlersLength; i++)
        {
            ...
            // 每个EventHandler实现类都创建了一个对应的BatchEventProcessor,所有消费者共用一个SequenceBarrier(用于跟踪生产者游标,协调数据处理)
            ...
            // consumerRepository 这个类主要保存消费者的各种关系，如通过EventHandler引用获取EventProcessorInfo信息，通过Sequence获取ConsumerInfo信息等。
            consumerRepository.add(batchEventProcessor, eventHandler, barrier);
            processorSequences[i] = batchEventProcessor.getSequence();
        }

        // 每次添加完事件处理器后，更新序列，用于后续调用链的添加判断。指后续消费链的消费，不能超过前边。
